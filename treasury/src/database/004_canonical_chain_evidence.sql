...
-- CANONICAL is a claim about a specific block, so it cannot be asserted without
-- the hash that identifies it. This closes the path where a partial write, a
-- repair session or a future migration marks an entry canonical on height.
ALTER TABLE treasury_ledger_entries
...
ALTER TABLE treasury_ledger_entries
    ADD CONSTRAINT treasury_ledger_entries_canonical_requires_identity CHECK (
        ...
        OR (block_hash IS NOT NULL AND log_index IS NOT NULL AND canonicality_verified_at IS NOT NULL)
    );

-- Export and handoff scan for entries that are not provably canonical, and the
-- reorganization drill scans for the orphaned ones. Both read a small slice of
-- a table that is otherwise read by trade.
CREATE INDEX IF NOT EXISTS idx_treasury_ledger_entries_canonicality
    ON treasury_ledger_entries(canonicality_state, block_number);
...
-- idempotent, and the second pass is what backfills `block_hash` and
-- `log_index` onto the rows migrated above as UNVERIFIED.
ALTER TABLE treasury_ingestion_state
    ...
    ADD COLUMN IF NOT EXISTS last_finalized_block_number INT
...
COMMENT ON COLUMN treasury_ingestion_state.last_finalized_block_number IS
    'Finalized head the last completed run was bounded by, recorded as the stable block for reconciliation.';
