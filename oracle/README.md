...
| `ORACLE_SIGNER_CUSTODY_MODE`                                                                             | `raw_private_key` (default), `kms` for direct AWS KMS signing, or `mpc` for an HTTPS signer service                  |
| `ORACLE_KMS_KEY_ID`, `ORACLE_KMS_EXPECTED_ADDRESS`                                                       | AWS KMS key and independently approved address — required in `kms` mode; AWS credentials come from the task IAM role |
| `ORACLE_MANAGED_SIGNER_URL`, `ORACLE_MANAGED_SIGNER_API_KEY`, `ORACLE_MANAGED_SIGNER_REQUEST_TIMEOUT_MS` | Managed signer endpoint, bearer token, and request timeout — required only when custody mode is `mpc`                |
