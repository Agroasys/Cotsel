...
function main() {
  ...
  const servicesInScope = Array.from(mappingSection.matchAll(/^- `([^`]+)`$/gm), (match) =>
    match[1].trim(),
  );
  const missingServices = EXPECTED_SERVICES.filter((service) => !servicesInScope.includes(service));
  const unexpectedServices = servicesInScope.filter(
    (service) => !EXPECTED_SERVICES.includes(service),
  );
  ...
  const auditRequiredMatch = suppressionSection.match(
    /`suppression_audit_note_required`:\s*`([^`]+)`/,
  );
  ...
  if (!maxWindowMatch) {
    ...
    if (hours === null || hours <= 0 || hours > 2) {
      errors.push(
        '`max_suppression_window` must be a positive duration in hours up to 2h (example: `2h`).',
      );
    }
  }
