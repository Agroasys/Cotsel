...
test('readDashboardParitySessionArtifact rejects artifacts without a session id', () => {
  ...
  assert.throws(() => readDashboardParitySessionArtifact(artifactPath), /missing sessionId/);
});
...
test('dashboard parity failures render machine-usable codes and exit mappings', () => {
  ...
  const rendered = JSON.parse(
    formatDashboardParityFailure(failure, {
      gatewayBaseUrl: 'http://127.0.0.1:3600/api/dashboard-gateway/v1',
    }),
  );
