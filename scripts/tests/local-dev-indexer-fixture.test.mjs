...
test("normalizeLocalDevIndexerFixtureMode defaults to empty and rejects unknown modes", () => {
  ...
  assert.throws(() => normalizeLocalDevIndexerFixtureMode("random"), /Unsupported LOCAL_DEV_INDEXER_FIXTURE_MODE/);
});

test("loadLocalDevTradeFixtures returns the canonical parity trade only when parity mode is enabled", async () => {
  const emptyTrades = await loadLocalDevTradeFixtures({ fixtureMode: LOCAL_DEV_INDEXER_FIXTURE_EMPTY });
  assert.deepEqual(emptyTrades, []);

  const parityTrades = await loadLocalDevTradeFixtures({ fixtureMode: LOCAL_DEV_INDEXER_FIXTURE_DASHBOARD_PARITY });
...
test("buildLocalDevIndexerResponse preserves list and detail semantics for the seeded trade", async () => {
  const trades = await loadLocalDevTradeFixtures({ fixtureMode: LOCAL_DEV_INDEXER_FIXTURE_DASHBOARD_PARITY });
