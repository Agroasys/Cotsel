...
async function main() {
  ...
  const reconciliationCooldownMs = readNumberEnv(
    'RECONCILIATION_NOTIFICATIONS_COOLDOWN_MS',
    300000,
  );
  const reconciliationTimeoutMs = readNumberEnv(
  ...
  try {
    ...
    const reconciliationPayload = capture.capturedPayloads
      .map((entry) => entry.payload)
      .find((payload) =>
        String(payload?.text || '').includes(report.evidence.reconciliationEventType),
      );
