...
The hosted workflow requires an organization or repository secret named `CROSS_REPO_CHECKOUT_TOKEN` with read-only contents access to the three private sibling repositories. Use a dedicated fine-grained credential restricted to those repositories; it must not have package, write, administration or workflow authority. The private `@agroasys/sdk` package consumed by Cotsel-Dash is authenticated separately with the job-scoped `GITHUB_TOKEN`, and the package must grant the Cotsel repository Actions read access. The workflow is called automatically by `ci/release-gate` and fails closed when the checkout credential is absent; it may also be started manually to produce pinned compatibility evidence. `ci/release-gate` cannot pass until the checkout credential and package access are provisioned and the exact manifest-pinned checkouts pass. The Release Owner must retain this dependency in the protected aggregate gate and attach the first green compatibility run to the release record.
