...
| Governance signing model — human privileged governance → direct admin wallet signing | Base active | In Progress | 15 | #411 | `docs/adr/adr-0411-human-governance-direct-wallet-signing.md`, `docs/runbooks/gateway-governance-signer-custody.md` (transitional notice), `docs/api/cotsel-dashboard-gateway.openapi.yml` (description updated); supersedes Decision #215 | Phase 1 (gateway prepare endpoints), Phase 2 (dashboard wallet-sign flow), Phase 3 (auth step-up), Phase 4 (full runbook/OpenAPI update) — linked issues pending per ADR-0411 | roadmap-maintainers | 2026-04-05 | weekly |
