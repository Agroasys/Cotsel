...
| -------------- | -------------------------------------------------------------------- | -------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| auth           | `auth` container                                                     | `auth` container                       | `scripts/docker-services.sh health <profile>` (`/api/auth/v1/health`) + auth structured security events                                 |
...
| `reconciliation_freshness_stale`       | reconciliation | Latest completed run older than freshness SLO or stale RUNNING job        | CRITICAL | reconciliation DB freshness check + treasury eligibility block            | Freeze payout/export progression and inspect stuck runs       | Platform On-Call + Incident Commander | `scripts/staging-e2e-real-gate.sh && cat reports/reconciliation/staging-e2e-real-report.json`                                                                                                                                                                                                                                                                                         |
...
| `notifications_gate_failed`            | notifications  | Notification gate output fails deterministic route/template checks        | HIGH     | `scripts/notifications-gate.sh staging-e2e-real` + report JSON            | Rebuild notifications workspace and rerun gate                | Ops/Engineering On-Call               | `pnpm --filter ./notifications run build && scripts/notifications-gate.sh staging-e2e-real`                                                                                                                                                                                                                                                                                           |
...
| `auth_durable_admin_provisioned`       | auth           | `auth.durable_admin_provisioned` security event emitted                   | HIGH     | auth structured logs + `auth_admin_audit_events`                          | Verify ticket, approver, executor key ID, and target account  | Security Owner                        | `scripts/docker-services.sh logs staging-e2e-real auth`                                                                                                                                                                                                                                                                                                                               |
| `auth_durable_admin_revoked`           | auth           | `auth.durable_admin_revoked` security event emitted                       | HIGH     | auth structured logs + `auth_admin_audit_events`                          | Verify session revocation and access-review ticket            | Security Owner                        | `scripts/docker-services.sh logs staging-e2e-real auth`                                                                                                                                                                                                                                                                                                                               |
| `auth_break_glass_granted`             | auth           | `auth.break_glass_granted` security event emitted                         | CRITICAL | auth structured logs + `auth_admin_audit_events`                          | Open incident review, confirm TTL and planned revocation      | Incident Commander + Security Owner   | `scripts/docker-services.sh logs staging-e2e-real auth`                                                                                                                                                                                                                                                                                                                               |
| `auth_break_glass_revoked`             | auth           | `auth.break_glass_revoked` security event emitted                         | HIGH     | auth structured logs + `auth_admin_audit_events`                          | Verify temporary authority is gone and old sessions fail      | Security Owner                        | `scripts/docker-services.sh logs staging-e2e-real auth`                                                                                                                                                                                                                                                                                                                               |
| `auth_break_glass_expired`             | auth           | `auth.break_glass_expired` security event emitted                         | HIGH     | auth structured logs + `auth_admin_audit_events`                          | Verify temporary authority is gone and old sessions fail      | Security Owner                        | `scripts/docker-services.sh logs staging-e2e-real auth`                                                                                                                                                                                                                                                                                                                               |
| `auth_service_auth_denied`             | auth           | `auth.service_auth_denied` security event emitted                         | HIGH     | auth structured logs                                                      | Check key allowlist, source IP/path, and attempted endpoint   | Ops/Engineering On-Call               | `scripts/docker-services.sh logs staging-e2e-real auth`                                                                                                                                                                                                                                                                                                                               |
| `auth_nonce_replay_attempted`          | auth           | `auth.nonce_replay_attempted` security event emitted                      | HIGH     | auth structured logs + nonce-store rejection                              | Treat as suspected replay until explained                     | Security Owner                        | `scripts/docker-services.sh logs staging-e2e-real auth`                                                                                                                                                                                                                                                                                                                               |
| `auth_rate_limit_fail_closed`          | auth           | `auth.rate_limiter_fail_closed` security event emitted                    | HIGH     | auth structured logs                                                      | Verify Redis availability and blocked protected path          | Ops/Engineering On-Call               | `scripts/docker-services.sh logs staging-e2e-real auth`                                                                                                                                                                                                                                                                                                                               |
| `auth_or_gateway_rate_limit_fail_open` | auth           | `auth.rate_limiter_fail_open` or `gateway.rate_limiter_fail_open` emitted | CRITICAL | auth/gateway structured logs                                              | Confirm fail-open was explicitly configured and restore Redis | Incident Commander                    | `scripts/docker-services.sh logs staging-e2e-real auth && scripts/docker-services.sh logs staging-e2e-real gateway`                                                                                                                                                                                                                                                                   |
...
scripts/docker-services.sh logs staging-e2e-real auth | tail -n 20
