...
Before applying gateway migration `007`, inventory every existing governance
action with a non-null prepared payload. The migration must stop if any payload
lacks simulation evidence; do not manufacture or backfill evidence for an old
action. Allow an open preparation to expire or close it under an approved
incident/change record, preserve it as historical evidence, and only then apply
the reviewed migration. Before applying auth migration `003`, confirm that two
independent admin-control principals are available: legacy active signer
bindings deliberately become pending and authorize no session until a different
principal reviews and activates their exact evidence digest.
...
4. Review the returned chain, contract, signer, method, arguments, calldata,
   value, nonce, expiry, audit reference, prepared-payload hash, and both
   preparation and handoff simulation evidence. Confirm the simulation chain,
   block number, block hash, provider identity, timestamp, and successful result.
   Immediately before this response, the gateway must have re-resolved the Auth
   session and required the same actor and exact active signer binding. Simulation
   is point-in-time validation and does not guarantee future state.
...
The preparation expiry is enforced by Cotsel policy; it is not encoded as an
on-chain deadline. If a custodian broadcasts after expiry, submit the hash once
for detection. Cotsel records and monitors the exact late transaction but
rejects it as an approved confirmation and raises
`LATE_BROADCAST_REPORTED` until independently observed, then
`LATE_BROADCAST_DETECTED`. Freeze further governance actions, preserve the
action/transaction/audit evidence, and open the incident procedure. Do not
claim that the expired preparation invalidated the blockchain transaction.
...
address, initial and handoff simulation block/hash/provider/time/result evidence,
transaction hash, block number, receipt status, confirmation depth, final
monitoring state, late-broadcast incident flag where applicable, and reviewer
identities.
