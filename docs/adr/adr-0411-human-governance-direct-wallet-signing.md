...
Before returning a transaction for wallet review, the gateway performs an
`eth_call` simulation of the exact chain, sender, destination, value, calldata,
and nonce at an identified block, then re-resolves the authenticated session
from Auth and revalidates the same actor, exact active signer binding, chain,
contract, nonce, preparation validity, and exact call immediately before
handoff. The block hash, block number, provider identity, time, and result are
evidence of a
point-in-time check only. Simulation is not a cryptographic guarantee that
chain state will remain unchanged.

The short preparation TTL is an off-chain operator-policy boundary. It prevents
Cotsel from handing off or confirming an expired preparation, but cannot make a
valid signed blockchain transaction expire. A reported late broadcast is bound
to the action, monitored, and flagged as an incident; staging acceptance of
this residual risk belongs jointly to Security and Protocol. Cryptographic
expiry would require a separate contract/protocol change and deployment cycle.
