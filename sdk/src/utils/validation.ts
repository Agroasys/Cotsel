...
export function validateTradeParameters(params: BuyerLockPayload): void {
  ...
  if (params.logisticsAmount < 0n) {
    throw new ValidationError('logisticsAmount cannot be negative', {
      logisticsAmount: params.logisticsAmount,
    });
  }
  if (params.platformFeesAmount < 0n) {
    throw new ValidationError('platformFeesAmount cannot be negative', {
      platformFeesAmount: params.platformFeesAmount,
    });
  }
  if (params.supplierFirstTranche <= 0n) {
    throw new ValidationError('supplierFirstTranche must be positive', {
      supplierFirstTranche: params.supplierFirstTranche,
    });
  }
  if (params.supplierSecondTranche <= 0n) {
    throw new ValidationError('supplierSecondTranche must be positive', {
  ...
  if (params.totalAmount !== expectedTotal) {
    throw new ValidationError('amount breakdown does not match totalAmount', {
  ...
  if (!/^0x[a-fA-F0-9]{64}$/.test(params.ricardianHash)) {
    throw new ValidationError('ricardianHash must be a 32-byte hex string (0x...)', {
      ricardianHash: params.ricardianHash,
      length: params.ricardianHash.length,
    });
  }
}
