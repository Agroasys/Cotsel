...
describe('Escrow ABI alignment', () => {
  const abi = AgroasysEscrow__factory.abi;
  const asEvent = (value: JsonFragment | null | undefined): EventFragment | undefined => {
    if (!value || value.type !== 'event' || !value.name || !Array.isArray(value.inputs)) {
      return undefined;
    }

    return value as EventFragment;
  };
  ...
  test('OracleUpdateProposed includes emergencyFastTrack flag', () => {
    const event = asEvent(
      abi.find((item) => item.type === 'event' && item.name === 'OracleUpdateProposed'),
    );

    expect(event).toBeDefined();
    expect(event?.inputs.map((input) => input.type)).toEqual([
      'uint256',
      'address',
      'address',
      'uint256',
  ...
  test('TreasuryClaimed event has deterministic reconciliation fields', () => {
    const event = asEvent(
      abi.find((item) => item.type === 'event' && item.name === 'TreasuryClaimed'),
    );
