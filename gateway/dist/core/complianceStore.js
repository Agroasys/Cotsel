...
exports.ATTESTATION_FRESHNESS_STATES = exports.ATTESTATION_AVAILABILITIES = exports.ATTESTATION_REFERENCE_STATUSES = exports.ATTESTATION_ISSUER_KINDS = exports.COMPLIANCE_RISK_LEVELS = exports.COMPLIANCE_BLOCK_STATES = exports.COMPLIANCE_DECISION_RESULTS = exports.COMPLIANCE_DECISION_TYPES = void 0;
exports.encodeComplianceDecisionCursor = encodeComplianceDecisionCursor;
exports.decodeComplianceDecisionCursor = decodeComplianceDecisionCursor;
exports.createPostgresComplianceStore = createPostgresComplianceStore;
exports.createInMemoryComplianceStore = createInMemoryComplianceStore;
exports.COMPLIANCE_DECISION_TYPES = ['KYB', 'KYT', 'SANCTIONS'];
exports.COMPLIANCE_DECISION_RESULTS = ['ALLOW', 'DENY'];
exports.COMPLIANCE_BLOCK_STATES = ['not_blocked', 'blocked', 'resume_pending'];
exports.COMPLIANCE_RISK_LEVELS = ['low', 'medium', 'high', 'critical'];
exports.ATTESTATION_ISSUER_KINDS = ['provider', 'service', 'operator', 'partner'];
exports.ATTESTATION_REFERENCE_STATUSES = ['active', 'revoked', 'expired', 'unknown'];
exports.ATTESTATION_AVAILABILITIES = ['available', 'degraded', 'unavailable'];
exports.ATTESTATION_FRESHNESS_STATES = ['current', 'stale', 'expired', 'unknown'];
function cloneAttestationRecord(attestation) {
    return {
        ...attestation,
        issuer: {
...
function cloneEvidenceLinks(links) {
...
function cloneAuditRecord(audit) {
    return {
        ...audit,
...
function cloneDecisionRecord(decision) {
    return {
        ...decision,
...
function cloneBlockRecord(block) {
    return {
        ...block,
        audit: cloneAuditRecord(block.audit),
    };
}
function mapDecisionRow(row, blockState) {
    return {
...
function encodeComplianceDecisionCursor(cursor) {
    return Buffer.from(JSON.stringify(cursor), 'utf8').toString('base64url');
}
function decodeComplianceDecisionCursor(cursor) {
...
function blockedFlag(state) {
    return state === 'blocked' || state === 'resume_pending';
}
async function loadBlockState(pool, tradeId) {
...
function createPostgresComplianceStore(pool) {
    ...
    return {
        async saveDecision(decision) {
            await pool.query(`INSERT INTO compliance_decisions (
          decision_id,
          trade_id,
          decision_type,
          result,
        ...
        async getDecision(decisionId) {
            const result = await pool.query(`${selectDecisionColumns}
        ...
        async saveOracleProgressionBlock(block) {
            await pool.query(`INSERT INTO oracle_progression_blocks (
          trade_id,
          latest_decision_id,
          block_state,
          reason_code,
        ...
        async countBlockedTrades() {
            const result = await pool.query(`SELECT COUNT(*)::text AS count FROM oracle_progression_blocks WHERE block_state IN ('blocked', 'resume_pending')`);
            return Number.parseInt(result.rows[0]?.count ?? '0', 10);
        },
    };
}
function createInMemoryComplianceStore(initialDecisions = [], initialBlocks = []) {
    const decisions = new Map(initialDecisions.map((decision) => [decision.decisionId, cloneDecisionRecord(decision)]));
    const blocks = new Map(initialBlocks.map((block) => [block.tradeId, cloneBlockRecord(block)]));
    function sortedTradeDecisions(tradeId) {
    ...
    return {
        ...
        async listTradeDecisions(input) {
            ...
            if (input.cursor) {
                const cursor = decodeComplianceDecisionCursor(input.cursor);
                candidates = candidates.filter((decision) => (decision.decidedAt < cursor.decidedAt
        ...
        async countBlockedTrades() {
            return [...blocks.values()].filter((block) => block.blockState === 'blocked' || block.blockState === 'resume_pending').length;
        },
    };
}
//# sourceMappingURL=complianceStore.js.map