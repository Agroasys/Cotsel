...
exports.SETTLEMENT_EVENT_TYPES = exports.SETTLEMENT_CALLBACK_STATUSES = exports.SETTLEMENT_RECONCILIATION_STATUSES = exports.SETTLEMENT_EXECUTION_STATUSES = void 0;
exports.createPostgresSettlementStore = createPostgresSettlementStore;
exports.createGatewayServiceAuthNonceStore = createGatewayServiceAuthNonceStore;
exports.createInMemorySettlementStore = createInMemorySettlementStore;
/**
...
exports.SETTLEMENT_EXECUTION_STATUSES = [
    'pending',
    'accepted',
    'queued',
    'submitted',
...
exports.SETTLEMENT_RECONCILIATION_STATUSES = [
    'pending',
    'matched',
    'drift',
    'unavailable',
];
exports.SETTLEMENT_CALLBACK_STATUSES = [
    'pending',
    'delivered',
    'failed',
    'dead_letter',
...
exports.SETTLEMENT_EVENT_TYPES = [
    'accepted',
    'queued',
    'submitted',
    'confirmed',
...
function parseDecimal(value) {
    ...
    if (!Number.isFinite(parsed)) {
        throw new errors_1.GatewayError(500, 'INTERNAL_ERROR', 'Stored settlement numeric value is invalid', { value });
    }
    return parsed;
}
...
function buildHandoffFilterClause(input) {
    const filters = [];
    const params = [];
    if (input.tradeId) {
...
async function cleanupExpiredNonces(pool) {
    await pool.query('DELETE FROM service_auth_nonces WHERE expires_at < NOW()');
}
async function createEventWithClient(client, input) {
    const eventId = (0, crypto_1.randomUUID)();
    const insertEvent = await client.query(`INSERT INTO settlement_execution_events (
       event_id,
       handoff_id,
       event_type,
       execution_status,
    ...
    if (!event) {
        throw new errors_1.GatewayError(500, 'INTERNAL_ERROR', 'Failed to persist settlement execution event');
    }
    await client.query(`UPDATE settlement_handoffs
...
function createPostgresSettlementStore(pool) {
    const getHandoff = async (handoffId) => {
    ...
    const getHandoffByPlatformRef = async (platformId, platformHandoffId) => {
        const result = await pool.query(`SELECT
         handoff_id AS "handoffId",
         platform_id AS "platformId",
         platform_handoff_id AS "platformHandoffId",
         ...
         updated_at AS "updatedAt"
       FROM settlement_handoffs
       WHERE platform_id = $1 AND platform_handoff_id = $2`, [platformId, platformHandoffId]);
        return result.rows[0] ? mapHandoffRow(result.rows[0]) : null;
    };
    return {
        async createHandoff(input) {
            ...
            const result = await pool.query(`INSERT INTO settlement_handoffs (
           handoff_id,
           platform_id,
           platform_handoff_id,
           trade_id,
         ...
         RETURNING
           ...
           updated_at AS "updatedAt"`, [
            ...
            if (!row) {
                throw new errors_1.GatewayError(500, 'INTERNAL_ERROR', 'Failed to persist settlement handoff');
            }
        ...
        async listHandoffs(input) {
           ...
           OFFSET $${params.length + 2}`, [...params, String(input.limit), String(input.offset)]),
                pool.query(`SELECT
             COUNT(*)::text AS total,
           ...
           ${clause}`, params),
            ...
            if (!Number.isFinite(total)) {
                throw new errors_1.GatewayError(500, 'INTERNAL_ERROR', 'Stored settlement total is invalid', {
                    total: summary?.total,
                });
            }
            return {
        ...
        async createExecutionEvent(input) {
            ...
            try {
                ...
                const handoffCheck = await client.query(`SELECT
             handoff_id AS "handoffId",
             execution_status AS "executionStatus"
           FROM settlement_handoffs
           WHERE handoff_id = $1
           FOR UPDATE`, [input.handoffId]);
                ...
                (0, settlementStateMachine_1.validateExecutionTransition)(handoffCheck.rows[0].executionStatus, input.executionStatus, input.eventType);
        ...
        async listExecutionEvents(handoffId) {
         ...
         WHERE handoff_id = $1
         ORDER BY observed_at DESC, event_id DESC`, [handoffId]);
            return result.rows.map(mapEventRow);
        },
        async queueCallbackDelivery(input) {
            const deliveryId = (0, crypto_1.randomUUID)();
            const result = await pool.query(`INSERT INTO settlement_callback_deliveries (
           delivery_id,
           handoff_id,
           event_id,
           target_url,
         ...
         RETURNING
           ...
           updated_at AS "updatedAt"`, [
            ...
            if (input.status === 'disabled') {
                await pool.query(`UPDATE settlement_handoffs
           SET callback_status = 'disabled',
               updated_at = NOW()
           WHERE handoff_id = $1`, [input.handoffId]);
            }
        ...
        async markCallbackDelivering(deliveryId, attemptedAt) {
            const result = await pool.query(`UPDATE settlement_callback_deliveries
         ...
         RETURNING
           ...
           request_id AS "requestId",
           created_at AS "createdAt",
           updated_at AS "updatedAt"`, [deliveryId, attemptedAt]);
            return result.rows[0] ? mapDeliveryRow(result.rows[0]) : null;
        },
        async markCallbackDelivered(deliveryId, completedAt, responseStatus) {
            await pool.query(`UPDATE settlement_callback_deliveries
         SET status = 'delivered',
             response_status = $3,
             delivered_at = $2,
             updated_at = NOW()
         WHERE delivery_id = $1`, [deliveryId, completedAt, responseStatus]);
            await pool.query(`UPDATE settlement_handoffs handoffs
         ...
         WHERE deliveries.delivery_id = $1
           ...
           AND handoffs.latest_event_id = deliveries.event_id`, [deliveryId, completedAt]);
        },
        async markCallbackFailed(deliveryId, update) {
         ...
         WHERE deliveries.delivery_id = $1
           AND handoffs.handoff_id = deliveries.handoff_id
           AND handoffs.latest_event_id = deliveries.event_id`, [deliveryId, update.deadLetter]);
        },
        async requeueCallbackDelivery(deliveryId, nextAttemptAt) {
         ...
         RETURNING
           ...
           request_id AS "requestId",
           created_at AS "createdAt",
           updated_at AS "updatedAt"`, [deliveryId, nextAttemptAt]);
            return result.rows[0] ? mapDeliveryRow(result.rows[0]) : null;
        },
        async getTradeSettlementProjectionMap(tradeIds) {
            ...
            const result = await pool.query(`SELECT DISTINCT ON (trade_id)
           handoff_id AS "handoffId",
           platform_id AS "platformId",
           platform_handoff_id AS "platformHandoffId",
           trade_id AS "tradeId",
         ...
         ORDER BY trade_id, updated_at DESC, handoff_id DESC`, [tradeIds]);
            const map = new Map();
...
function createGatewayServiceAuthNonceStore(pool) {
    return {
        async consume(apiKey, nonce, ttlSeconds) {
            await cleanupExpiredNonces(pool);
            const expiresAt = new Date(Date.now() + ttlSeconds * 1000).toISOString();
            const result = await pool.query(`INSERT INTO service_auth_nonces (api_key, nonce, expires_at)
         VALUES ($1, $2, $3)
         ON CONFLICT (api_key, nonce) DO NOTHING`, [apiKey, nonce, expiresAt]);
...
function createInMemorySettlementStore(initialHandoffs = []) {
    ...
    const byTrade = (tradeId) => [...handoffs.values()]
