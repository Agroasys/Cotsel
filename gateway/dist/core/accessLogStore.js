...
exports.ACCESS_AUDIT_REFERENCE_TYPES = void 0;
exports.encodeAccessLogCursor = encodeAccessLogCursor;
exports.decodeAccessLogCursor = decodeAccessLogCursor;
exports.createPostgresAccessLogStore = createPostgresAccessLogStore;
exports.createInMemoryAccessLogStore = createInMemoryAccessLogStore;
exports.ACCESS_AUDIT_REFERENCE_TYPES = [
    'audit_log',
    'governance_action',
    'compliance_decision',
    'settlement_handoff',
...
function cloneAuditReferences(references) {
...
function encodeAccessLogCursor(cursor) {
    return Buffer.from(JSON.stringify(cursor), 'utf8').toString('base64url');
}
function decodeAccessLogCursor(cursor) {
...
function createPostgresAccessLogStore(pool) {
    ...
    return {
        async append(entry) {
            await pool.query(`INSERT INTO access_log_entries (
          entry_id,
          event_type,
          surface,
          outcome,
...
function createInMemoryAccessLogStore(initial = []) {
    const items = new Map(initial.map((entry) => [entry.entryId, cloneEntry(entry)]));
    function sorted() {
    ...
    return {
        ...
        async list(input) {
            ...
            if (input.cursor) {
                const cursor = decodeAccessLogCursor(input.cursor);
                candidates = candidates.filter((entry) => (entry.createdAt < cursor.createdAt
...
//# sourceMappingURL=accessLogStore.js.map