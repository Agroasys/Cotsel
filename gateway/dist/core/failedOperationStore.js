...
exports.FailedOperationConflictError = void 0;
exports.createPostgresFailedOperationStore = createPostgresFailedOperationStore;
exports.createInMemoryFailedOperationStore = createInMemoryFailedOperationStore;
/**
 * SPDX-License-Identifier: Apache-2.0
 */
const crypto_1 = __importDefault(require("crypto"));
class FailedOperationConflictError extends Error {
    constructor(message) {
        super(message);
        this.name = 'FailedOperationConflictError';
    }
}
exports.FailedOperationConflictError = FailedOperationConflictError;
function mapRow(row) {
    return {
...
function buildPayloadHash(requestPayload) {
...
function toScopedKey(operationType, operationKey) {
    return `${operationType}\u0000${operationKey}`;
}
function createPostgresFailedOperationStore(pool) {
    return {
        async recordFailure(input) {
            const result = await pool.query(`INSERT INTO failed_operations (
           operation_type,
           operation_key,
           target_service,
           route,
         ...
         RETURNING
           ...
           updated_at AS "updatedAt"`, [
            ...
            if (!result.rows[0]) {
                throw new FailedOperationConflictError('Failed operation key was reused with a conflicting payload');
            }
        ...
        async get(failedOperationId) {
         ...
         WHERE failed_operation_id = $1`, [failedOperationId]);
        ...
        async list(input = {}) {
         ...
         FROM failed_operations
         ${whereClause}
         ORDER BY last_failed_at DESC, failed_operation_id DESC`, params);
            return result.rows.map(mapRow);
        },
        async markReplayed(failedOperationId, replayedAt, metadata = {}) {
            await pool.query(`UPDATE failed_operations
         SET failure_state = 'replayed',
             last_replayed_at = $2,
             metadata = metadata || $3::jsonb,
             updated_at = NOW()
         WHERE failed_operation_id = $1`, [failedOperationId, replayedAt, JSON.stringify(metadata)]);
        },
...
function createInMemoryFailedOperationStore(initialRecords = []) {
...
//# sourceMappingURL=failedOperationStore.js.map