...
exports.createPostgresComplianceWriteStore = createPostgresComplianceWriteStore;
exports.createPassthroughComplianceWriteStore = createPassthroughComplianceWriteStore;
...
async function insertComplianceDecision(client, decision) {
    await client.query(`INSERT INTO compliance_decisions (
      decision_id,
      trade_id,
      decision_type,
      result,
...
async function upsertOracleProgressionBlock(client, block) {
    await client.query(`INSERT INTO oracle_progression_blocks (
      trade_id,
      latest_decision_id,
      block_state,
      reason_code,
...
async function runTransactionalWrite(pool, operation) {
...
function createPostgresComplianceWriteStore(pool, store) {
    return {
...
function createPassthroughComplianceWriteStore(store, auditLogStore) {
    return {
        async saveDecisionWithAudit(decision, auditEntry) {
...
//# sourceMappingURL=complianceWriteStore.js.map