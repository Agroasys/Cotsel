...
const overviewFixture: OverviewSnapshot = {
  ...
  feedFreshness: {
    trades: {
      ...
      state: 'current',
      queriedAt: '2026-03-09T00:00:05.000Z',
    ...
    governance: {
      source: 'chain_rpc',
      state: 'current',
      queriedAt: '2026-03-09T00:00:00.000Z',
      freshAt: '2026-03-09T00:00:00.000Z',
      available: true,
    },
    compliance: {
...
async function startServer(
  ...
  fixture: OverviewSnapshot = overviewFixture,
) {
...
describe('gateway overview route contract', () => {
  ...
  test('GET /overview returns schema-valid snapshot', async () => {
    ...
    try {
      ...
      expect(payload.data.feedFreshness.trades.state).toBe('current');
