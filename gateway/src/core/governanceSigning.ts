...
type GovernanceSigningPayloadDraft = Omit<
  GovernancePreparedSigningPayload,
  'simulation' | 'preparedPayloadHash'
>;
...
function buildPreparedPayloadHash(
  ...
  const canonical = [
    ...
    [
      payload.simulation.chainId,
      payload.simulation.blockNumber,
      payload.simulation.blockHash,
      payload.simulation.simulatedAt,
      payload.simulation.providerIdentity,
      payload.simulation.result,
      payload.simulation.returnDataHash,
      payload.simulation.pointInTimeOnly,
    ],
  ];
  return createHash('sha256').update(JSON.stringify(canonical)).digest('hex');
}

export function buildSigningPayloadDraft(
  config: GatewayConfig,
  input: {
    actionId: string;
    intentKey: string;
...
export function finalizeSigningPayload(
  draft: GovernanceSigningPayloadDraft,
  simulation: GovernanceSimulationEvidence,
): GovernancePreparedSigningPayload {
  const payload = { ...draft, simulation };
...
export function toPreparedResponse(
  ...
  handoffSimulation: GovernanceSimulationEvidence,
): GovernanceActionPrepared {
  if (
...
export function ensurePreparedSigningPayload(
  ...
  const simulationValid =
    signing.simulation?.chainId === signing.chainId &&
    Number.isSafeInteger(signing.simulation.blockNumber) &&
    signing.simulation.blockNumber >= 0 &&
    /^0x[0-9a-f]{64}$/.test(signing.simulation.blockHash) &&
    Number.isFinite(Date.parse(signing.simulation.simulatedAt)) &&
    /^sha256:[0-9a-f]{64}$/.test(signing.simulation.providerIdentity) &&
    signing.simulation.result === 'success' &&
    /^[0-9a-f]{64}$/.test(signing.simulation.returnDataHash) &&
    signing.simulation.pointInTimeOnly === true;
