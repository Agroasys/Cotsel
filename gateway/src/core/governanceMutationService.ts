...
import {
  ...
  type GovernanceSimulationEvidence,
...
import type { GovernanceTransitionStore } from './governanceTransitionStore';
...
export class GovernanceMutationService {
  ...
  async prepareAction(input: PrepareGovernanceActionInput): Promise<GovernanceActionPrepared> {
    ...
    const signingDraft = buildSigningPayloadDraft(
      this.config,
      {
        actionId,
        intentKey,
    ...
    let initialSimulation: GovernanceSimulationEvidence;
    try {
      initialSimulation = await this.verifier.simulateTransaction(signingDraft.txRequest);
    } catch {
      throw new GatewayError(
        409,
        'CONFLICT',
        'Exact governance transaction simulation reverted or could not be proven',
        { actionId, chainId: signingDraft.chainId, contractAddress: signingDraft.contractAddress },
      );
    }
    const signing = finalizeSigningPayload(signingDraft, initialSimulation);
    ...
    if (!stored.expiresAt || stored.expiresAt <= new Date().toISOString()) {
      throw new GatewayError(
        409,
        'CONFLICT',
        'Governance preparation expired before wallet handoff',
        {
    ...
    if (
      storedSigning.chainId !== this.config.chainId ||
      normalizeAddressOrNull(this.config.escrowAddress) !== storedSigning.contractAddress
    ) {
      throw new GatewayError(
        409,
        'CONFLICT',
        'Governance chain or contract changed before wallet handoff',
        { actionId: stored.actionId },
      );
    }
    const handoffNonce = await this.verifier.getTransactionCount(storedSigning.signerWallet);
    if (handoffNonce !== storedSigning.txRequest.nonce) {
      throw new GatewayError(
        409,
        'CONFLICT',
        'Governance signer nonce changed before wallet handoff; prepare a new action',
        {
          actionId: stored.actionId,
          preparedNonce: storedSigning.txRequest.nonce,
          currentNonce: handoffNonce,
        },
      );
    }
    let handoffSimulation: GovernanceSimulationEvidence;
    try {
      handoffSimulation = await this.verifier.simulateTransaction(storedSigning.txRequest);
    } catch {
      throw new GatewayError(
        409,
        'CONFLICT',
        'Governance transaction revalidation failed before wallet handoff',
        { actionId: stored.actionId },
      );
    }
    if (handoffSimulation.chainId !== storedSigning.chainId) {
      throw new GatewayError(409, 'CONFLICT', 'Governance chain changed before wallet handoff', {
        actionId: stored.actionId,
      });
    }

    return toPreparedResponse(stored, storedSigning, handoffSimulation);
  }
  ...
  async confirmBroadcast(
    ...
    if (existing.status === 'broadcast') {
      ...
      if (existing.errorCode?.startsWith('LATE_BROADCAST_')) {
        throw new GatewayError(
          409,
          'CONFLICT',
          'Late governance transaction remains incident evidence and is not an approved confirmation',
          { actionId: existing.actionId, txHash: existing.txHash, expiresAt: existing.expiresAt },
        );
      }
    ...
    const lateBroadcast = Boolean(existing.expiresAt && existing.expiresAt <= confirmedAt);
    const broadcastAt = existing.broadcastAt ?? confirmedAt;
    const lateBroadcastCode =
      lateBroadcast && verification.verificationState === 'verified'
        ? 'LATE_BROADCAST_DETECTED'
        : lateBroadcast
          ? 'LATE_BROADCAST_REPORTED'
          : existing.errorCode;
    ...
    const updatedAction: GovernanceActionRecord = {
      ...
      errorCode: lateBroadcastCode,
      errorMessage: lateBroadcast
        ? verification.verificationState === 'verified'
          ? 'Transaction was observed after the off-chain preparation policy expired'
          : 'Transaction hash was reported after the off-chain preparation policy expired'
    ...
    const auditEntry: AuditLogEntry = {
      ...
      metadata: {
        ...
        breakGlassReviewStatus: input.principal.session.breakGlass?.reviewStatus ?? null,
        lateBroadcast,
        preparationExpiredAt: existing.expiresAt,
      },
    };

    const stored = await this.transitionStore.commitConfirmation({
      actionId: input.actionId,
      transactionHash: input.txHash,
      signerBindingId: input.signerBinding.bindingId,
      signerWallet: expectedSigning.signerWallet,
      preparedPayloadHash: expectedSigning.preparedPayloadHash,
      committedAt: confirmedAt,
      transition: updatedAction,
      auditEntry,
      lateBroadcast,
    });
    if (lateBroadcast) {
      throw new GatewayError(
        ...
        'CONFLICT',
        verification.verificationState === 'verified'
          ? 'Late governance broadcast detected and recorded for incident review'
          : 'Late governance transaction hash reported and recorded for verification',
        { actionId: stored.actionId, txHash: stored.txHash, expiresAt: stored.expiresAt },
      );
    }
