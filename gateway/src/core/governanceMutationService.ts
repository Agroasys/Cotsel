...
function buildPreparedPayloadHash(
  payload: Pick<
    GovernancePreparedSigningPayload,
    'chainId' | 'contractAddress' | 'contractMethod' | 'args' | 'txRequest' | 'signerWallet'
  >,
): string {
...
export class GovernanceMutationService {
  ...
  async queueAction(input: QueueGovernanceActionInput): Promise<GovernanceMutationAccepted> {
    ...
    const expiresAt = new Date(
      Date.parse(acceptedAt) + this.config.governanceQueueTtlSeconds * 1000,
    ).toISOString();
  ...
  async prepareAction(input: PrepareGovernanceActionInput): Promise<GovernanceActionPrepared> {
    ...
    const stored = saved.created
      ? record
      : ((await this.actionStore.get(saved.action.actionId)) ?? saved.action);
    const storedSigning = ensurePreparedSigningPayload(
      this.config,
      stored,
      stored.audit.actorWallet || signerWallet,
    );
  ...
  private async verifyBroadcast(
    ...
    if (!actualFrom) {
      throw new GatewayError(
        409,
        'CONFLICT',
        'Broadcast transaction signer could not be resolved',
        {
          actionId: existing.actionId,
        },
      );
  ...
  async confirmBroadcast(
    ...
    const expectedSigning = ensurePreparedSigningPayload(
      this.config,
      existing,
      existing.audit.actorWallet || confirmingWallet,
    );
    ...
    const actorId = resolveGovernanceActorId(input.principal);
    const verification = await this.verifyBroadcast(
      existing,
      input.txHash,
