    ...
    async () => {
      await withPostgres(async (pool) => {
        ...
        try {
          ...
          const signerApprovalPath = '/api/auth/v1/admin/signers/approve';
          const signerApprovalBody = JSON.stringify({
            bindingId: '00000000-0000-0000-0000-000000000001',
            evidenceDigest: 'a'.repeat(64),
            reason: 'Attempt signer approval without an authorized control principal.',
          });
          const deniedSignerApproval = await fetch(`${app.baseUrl}${signerApprovalPath}`, {
            method: 'POST',
            headers: {
              ...signedHeaders({
                method: 'POST',
                path: signerApprovalPath,
                body: signerApprovalBody,
                nonce: 'nonce-denied-signer-approval',
              }),
              'X-Api-Key': 'not-allowed-for-admin-control',
            },
            body: signerApprovalBody,
          });
          expect(deniedSignerApproval.status).toBe(401);
