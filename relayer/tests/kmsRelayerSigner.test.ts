...
test('uses only the configured secp256k1 KMS identity to sign the approved transaction', async () => {
  ...
  const signInput = (send.mock.calls[1][0] as SignCommand).input;
  expect(signInput.KeyId).toBe(config.kmsKeyId);
  expect(signInput.MessageType).toBe(MessageType.DIGEST);
  expect(signInput.SigningAlgorithm).toBe(SigningAlgorithmSpec.ECDSA_SHA_256);
});
