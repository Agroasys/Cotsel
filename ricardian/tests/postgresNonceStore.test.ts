...
describe('postgres nonce store', () => {
  ...
  test('nonce remains rejected after restart simulation until ttl expires', async () => {
    const state: StoreState = { nowMs: 1_700_000_000_000, entries: new Map() };
    const query = createQuery(state);
    ...
    await expect(firstInstance.consume('svc-a', 'nonce-1', 2)).resolves.toBe(true);

    const restartedInstance = createPostgresNonceStore({
      tableName: 'ricardian_auth_nonces',
      query,
    });
    await expect(restartedInstance.consume('svc-a', 'nonce-1', 2)).resolves.toBe(false);

    state.nowMs += 2_500;
    await expect(restartedInstance.consume('svc-a', 'nonce-1', 2)).resolves.toBe(true);
  });
});
