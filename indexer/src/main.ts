    ...
    for (const log of block.logs) {
      ...
      try {
        ...
        switch (decoded.name) {
          // Trade events
          case 'TradeLocked':
            overviewSnapshot = await handleTradeLocked(
              decoded,
              trades,
              tradeEvents,
              overviewSnapshot,
          ...
          case 'FundsReleasedStage1':
            overviewSnapshot = await handleFundsReleasedStage1(
              decoded,
              trades,
              tradeEvents,
              overviewSnapshot,
          ...
          case 'PlatformFeesPaidStage1':
            overviewSnapshot = await handlePlatformFeesPaidStage1(
              decoded,
              trades,
              tradeEvents,
              overviewSnapshot,
          ...
          case 'DisputeOpenedByBuyer':
            overviewSnapshot = await handleDisputeOpenedByBuyer(
              decoded,
              trades,
              tradeEvents,
              overviewSnapshot,
          ...
          case 'TradeCancelledAfterLockTimeout':
            overviewSnapshot = await handleTradeCancelledAfterLockTimeout(
              decoded,
              trades,
              tradeEvents,
              overviewSnapshot,
          ...
          case 'DisputeSolutionProposed':
            await handleDisputeSolutionProposed(
              decoded,
              trades,
              disputeProposals,
              disputeEvents,
          ...
          case 'DisputeProposalExpiredCancelled':
            await handleDisputeProposalExpiredCancelled(
              decoded,
              disputeProposals,
              disputeEvents,
              eventId,
            ...
            break;
          case 'DisputePayout':
            overviewSnapshot = await handleDisputePayout(
              decoded,
              trades,
              tradeEvents,
              overviewSnapshot,
          ...
          case 'OracleUpdateProposed':
            await handleOracleUpdateProposed(
              decoded,
          ...
          case 'OracleUpdateApproved':
            await handleOracleUpdateApproved(
              decoded,
              oracleUpdateProposals,
              oracleEvents,
              eventId,
          ...
          case 'OracleUpdateProposalExpiredCancelled':
            await handleOracleUpdateProposalExpiredCancelled(
              decoded,
              oracleUpdateProposals,
              oracleEvents,
              eventId,
          ...
          case 'OracleDisabledEmergency':
            await handleOracleDisabledEmergency(
              decoded,
              oracleEvents,
              eventId,
              block,
          ...
          case 'UnpauseProposalCancelled':
            await handleUnpauseProposalCancelled(
              decoded,
              systemEvents,
              eventId,
              block,
          ...
          case 'TreasuryPayoutAddressUpdateProposed':
            await handleTreasuryPayoutAddressUpdateProposed(
              decoded,
              systemEvents,
              eventId,
              block,
          ...
          case 'TreasuryPayoutAddressUpdateApproved':
            await handleTreasuryPayoutAddressUpdateApproved(
              decoded,
              systemEvents,
              eventId,
              block,
          ...
          case 'TreasuryPayoutAddressUpdated':
            await handleTreasuryPayoutAddressUpdated(
              decoded,
              systemEvents,
              eventId,
              block,
...
async function handleTradeCancelledAfterLockTimeout(
  ...
  ctx.log.info(
    `Trade ${tradeId} cancelled after lock timeout - refunded ${refundedAmount} to ${buyer}`,
  );
  return overviewSnapshot;
}
...
function latestTerminalEventsByTradeId(events: TradeEvent[]): Map<string, string> {
  ...
  return new Map(
    Array.from(terminalEventByTradeId.entries()).map(([tradeId, event]) => {
      const key =
        event.eventName === 'DisputePayout' && event.payoutType === DisputeStatus.REFUND
...
function applySnapshotCounters(
  snapshot: OverviewSnapshot,
  counters: ReturnType<typeof snapshotCounters>,
) {
...
async function handleDisputeSolutionProposed(
  ...
  ctx.log.info(
    `Dispute solution proposed: proposal ${proposalId} for trade ${tradeId} with status ${disputeStatusEnum}`,
  );
}
...
async function handleDisputeApproved(
  ...
  ctx.log.info(
    `Dispute proposal ${proposalId} approved by ${approver} - ${approvalCount}/${requiredApprovals}`,
  );
}
...
async function handleDisputeFinalized(
  ...
  ctx.log.info(
    `Dispute ${proposalId} finalized for trade ${tradeId} with status ${disputeStatusEnum}`,
  );
}
...
async function handleDisputeProposalExpiredCancelled(
  ...
  ctx.log.info(
    `Dispute proposal ${proposalId} for trade ${tradeId} expired and cancelled by ${cancelledBy}`,
  );
}
...
async function handleDisputePayout(
  ...
  const counters =
    payoutTypeEnum === DisputeStatus.REFUND
  ...
  ctx.log.info(
    `Dispute payout for trade ${tradeId}: ${amount} to ${recipient} (type: ${payoutTypeEnum})`,
  );
  return overviewSnapshot;
}
...
async function handleOracleUpdateProposed(
  ...
  ctx.log.info(
    `Oracle update proposed: ${proposalId} to ${newOracle} by ${proposer} (fastTrack=${emergencyFastTrack})`,
  );
}
...
async function handleOracleDisabledEmergency(
  ...
  ctx.log.info(
    `Oracle disabled in emergency by ${disabledBy} - previous oracle: ${previousOracle}`,
  );
}
