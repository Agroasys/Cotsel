module.exports = class EventHashSeparation1771815600000 {
  ...
  async up(db) {
    ...
    await db.query(
      `CREATE INDEX IF NOT EXISTS "IDX_trade_event_extrinsic_hash" ON "trade_event" ("extrinsic_hash")`,
    );
    await db.query(
      `CREATE INDEX IF NOT EXISTS "IDX_dispute_event_extrinsic_hash" ON "dispute_event" ("extrinsic_hash")`,
    );
    await db.query(
      `CREATE INDEX IF NOT EXISTS "IDX_oracle_event_extrinsic_hash" ON "oracle_event" ("extrinsic_hash")`,
    );
    await db.query(
      `CREATE INDEX IF NOT EXISTS "IDX_admin_event_extrinsic_hash" ON "admin_event" ("extrinsic_hash")`,
    );
    await db.query(
      `CREATE INDEX IF NOT EXISTS "IDX_system_event_extrinsic_hash" ON "system_event" ("extrinsic_hash")`,
    );
  }
