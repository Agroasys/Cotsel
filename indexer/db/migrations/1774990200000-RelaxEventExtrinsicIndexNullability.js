module.exports = class RelaxEventExtrinsicIndexNullability1774990200000 {
  ...
  async down(db) {
    await db.query(
      `UPDATE "trade_event" SET "extrinsic_index" = 0 WHERE "extrinsic_index" IS NULL`,
    );
    await db.query(
      `UPDATE "dispute_event" SET "extrinsic_index" = 0 WHERE "extrinsic_index" IS NULL`,
    );
    await db.query(
      `UPDATE "oracle_event" SET "extrinsic_index" = 0 WHERE "extrinsic_index" IS NULL`,
    );
    await db.query(
      `UPDATE "admin_event" SET "extrinsic_index" = 0 WHERE "extrinsic_index" IS NULL`,
    );
    await db.query(
      `UPDATE "system_event" SET "extrinsic_index" = 0 WHERE "extrinsic_index" IS NULL`,
    );
